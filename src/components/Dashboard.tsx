...
export default function Dashboard({ onNavigateToCalculator, onNavigateToGSTRegistration, onNavigateToIRISProfile }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-slate-50/30">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="relative bg-gradient-to-br from-blue-600 via-cyan-600 to-teal-600 rounded-3xl p-6 sm:p-8 mb-6 sm:mb-8 overflow-hidden shadow-2xl animate-[fadeInSlideUp_0.6s_ease-out]">
          ...
          <div className="absolute -bottom-8 -left-8 w-40 h-40 bg-cyan-400/20 rounded-full blur-2xl animate-[pulse_5s_ease-in-out_infinite]"></div>

          <div className="relative flex flex-wrap items-center gap-2 mb-6 sm:mb-0 sm:absolute sm:top-6 sm:left-6 z-10">
            <span className="px-4 py-1.5 bg-white/95 backdrop-blur-sm text-blue-700 text-xs font-bold rounded-full shadow-lg animate-[bounceIn_0.8s_ease-out]">
              Tax Year 2025
            </span>
            <span className="px-4 py-1.5 bg-blue-700/80 backdrop-blur-sm text-white text-xs font-bold rounded-full shadow-lg animate-[bounceIn_0.9s_ease-out]">
              Active
            </span>
          </div>
          ...
          <div className="relative sm:mt-10 z-10">
            ...
            <p className="text-sm sm:text-lg text-blue-50 mb-5 font-medium drop-shadow animate-[fadeInSlideRight_0.9s_ease-out]">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-white/20 animate-[fadeInSlideRight_1s_ease-out]">
              <span className="text-sm font-bold text-amber-300 drop-shadow">⏰ Hurry up</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className="absolute -top-24 -right-24 w-48 h-48 bg-gradient-to-br from-blue-400/10 to-cyan-400/10 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700"></div>

              <div className="relative flex items-start justify-between mb-4">
                <div className="flex-1">
                  <p className="text-xs sm:text-sm font-bold text-gray-500 uppercase tracking-wide mb-2 group-hover:text-blue-600 transition-colors">{stat.label}</p>
                  <p className="text-3xl sm:text-4xl font-bold bg-gradient-to-br from-gray-900 to-gray-700 bg-clip-text text-transparent mb-1 group-hover:from-blue-600 group-hover:to-cyan-600 transition-all duration-300">
                    {loading ? (
                ...
                <div className="relative">
                  <div className={`absolute inset-0 ${stat.color} rounded-2xl blur opacity-20 group-hover:opacity-40 transition-opacity duration-300`}></div>
                  <div className={`relative w-14 h-14 sm:w-16 sm:h-16 ${stat.color} rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
              ...
              <div className="relative h-1.5 w-full bg-gray-100 rounded-full mb-3 overflow-hidden">
                <div className={`h-full ${stat.color.replace('bg-gradient-to-br', 'bg-gradient-to-r')} rounded-full transition-all duration-1000 ${loading ? 'w-0' : 'w-full'}`}></div>
              </div>
        ...
        <div className="relative bg-white rounded-3xl shadow-xl border border-gray-100 p-5 sm:p-7 lg:p-9 mb-6 sm:mb-8 overflow-hidden">
          ...
          <div className="relative flex items-center gap-3 mb-6 sm:mb-8">
            <div className="relative group">
              <div className="absolute inset-0 bg-gradient-to-br from-blue-400 to-cyan-500 rounded-xl blur opacity-40 group-hover:opacity-60 transition-opacity"></div>
            ...
            {services.map((service, index) => (
              <div
                ...
                className={`group relative bg-gradient-to-br from-white to-gray-50 border-2 rounded-2xl p-5 sm:p-6 transition-all duration-500 overflow-hidden ${
                  service.available
                    ? 'border-gray-200 hover:border-blue-400 hover:shadow-2xl cursor-pointer hover:-translate-y-2 hover:from-blue-50/30 hover:to-cyan-50/30'
              ...
              >
                ...
                <div className="absolute -top-20 -right-20 w-40 h-40 bg-gradient-to-br from-blue-400/0 to-cyan-400/0 group-hover:from-blue-400/20 group-hover:to-cyan-400/20 rounded-full blur-2xl transition-all duration-700"></div>
                ...
                <div className="relative mb-5">
                  <div className="relative">
                    <div className={`absolute inset-0 ${service.iconBg} rounded-2xl blur opacity-20 group-hover:opacity-40 transition-opacity`}></div>
                    <div className={`relative w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center shadow-lg ${
                      service.iconBg
                    } group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
                      <service.icon className={`w-7 h-7 sm:w-8 sm:h-8 ${service.iconColor || 'text-white'}`} />
                    </div>
                  </div>
                </div>

                <h4 className="relative text-base sm:text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors duration-300">{service.title}</h4>
                <p className="relative text-xs sm:text-sm text-gray-600 leading-relaxed">{service.description}</p>

                {service.available && (
                  <div className="relative mt-4 pt-4 border-t border-gray-200 group-hover:border-blue-200 transition-colors">
                    <span className="inline-flex items-center gap-1 text-xs font-bold text-blue-600 group-hover:text-blue-700 group-hover:gap-2 transition-all">
