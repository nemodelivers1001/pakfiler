/*
  ...
  - `status` (text) - Application status: draft, pending_payment, payment_verified, processing, completed, rejected
...
-- Create storage bucket for GST documents
INSERT INTO storage.buckets (id, name, public)
VALUES ('gst-documents', 'gst-documents', false)
...
TO authenticated
WITH CHECK (
  bucket_id = 'gst-documents' AND
  (storage.foldername(name))[1] IN (
    SELECT id::text FROM gst_applications WHERE user_id = auth.uid()
  )
);
